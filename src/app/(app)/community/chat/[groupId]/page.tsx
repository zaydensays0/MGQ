...
import { ArrowLeft, Send, Users, MoreVertical, Trash2, UserPlus, AlertTriangle } from 'lucide-react';
...
function ChatView() {
    ...
    const { getGroupById, addMessageToGroup } = useGroups();
    const { user } = useUser();
        ...
        return (
            <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <Button asChild variant="outline" className="mt-4">
                    <Link href="/community"><ArrowLeft className="mr-2 h-4 w-4" /> Back to Groups</Link>
                </Button>
            </div>
        );
    }
            ...
            <CardHeader className="flex flex-row items-center justify-between p-4 border-b">
                <div className="flex items-center space-x-4">
                    <Button variant="ghost" size="icon" className="h-8 w-8" asChild>
                        <Link href="/community"><ArrowLeft className="h-5 w-5" /></Link>
                    </Button>
                    <div>
                 ...
                 <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        <DropdownMenuItem disabled><UserPlus className="mr-2 h-4 w-4" /> Invite (Prototype)</DropdownMenuItem>
                        <DropdownMenuItem disabled><Users className="mr-2 h-4 w-4" /> View Members</DropdownMenuItem>
                        {isUserAdmin && <DropdownMenuItem disabled className="text-destructive focus:bg-destructive/10 focus:text-destructive"><Trash2 className="mr-2 h-4 w-4" /> Delete Group</DropdownMenuItem>}
                    </DropdownMenuContent>
                </DropdownMenu>
            </CardHeader>
